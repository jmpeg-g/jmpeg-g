...
public class FileChannelBitInputStream extends InputStream implements BitInputStream {
    ...
    public FileChannelBitInputStream(final FileChannel channel) throws IOException {
        this(channel, 64 * 1024 * 1024);
    }
    
    public FileChannelBitInputStream(final FileChannel channel, final int size) throws IOException {
       this.channel = channel;
       map(size);
    }
    
    private void map(final int size) throws IOException {
    ...
    public long getPosition() throws IOException {
        return channel.position() + buf.position() - (bits_left >>> 3);
    }
    ...
    public final long peekBits(final int nbits) throws IOException {
        ...
        final int next;
        if (total_bits >= nbits && buf.remaining() >= Short.BYTES) {
    ...
    public final long skip(final long nbytes) throws IOException {
        ...
        if (nbytes <= remaining) {
            total_bits -= nbytes * 8;
            buf.position(buf.position() + (int)nbytes);
            return nbytes;
        }
        
        final long position = channel.position();
        
        channel.position(position + buf.position() + nbytes);
        map(buf.limit());
        
        return channel.position() - position + buf.limit() - remaining; // CHECK!!!
    }
