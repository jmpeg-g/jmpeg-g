...
public abstract class AbstractAccessUnitEncoder {
    ...
    private final String[] readGroupIds;
    ...
    public AbstractAccessUnitEncoder(
        ...
        this.readGroupIds = encodingParameters.getReadGroupIds();
    }
    ...
    public void writeDescriptors(MPEGWriter output) throws IOException {
        for(int descriptor_i = 0; descriptor_i < symbols.length; descriptor_i++){
            if(descriptor_i == DESCRIPTOR_ID.RNAME.ID){
                ...
                AccessUnitBlock block = new AccessUnitBlock(
            ...
            final int subsequencesForDescriptor;
            if (descriptor_i == DESCRIPTOR_ID.QV.ID) {
                subsequencesForDescriptor = dataUnitParameters.getQualityValueParameterSet(auType).getNumberQualityBooks() + 2;
            } else {
                subsequencesForDescriptor = decoderConfiguration.getNumberSubsequences();
            }
            ...
            AccessUnitBlock block = new AccessUnitBlock(DESCRIPTOR_ID.getDescriptorId((byte) descriptor_i), payloads);
            block.write(output);
        }
    }
