...
package es.gencom.mpegg.encoder;
...
public class MappedAccessUnitEncoder extends AbstractAccessUnitEncoder{
    ...
    protected void writeSpecific(Record record) throws IOException {
        ...
        previousPosition = record.getMappingPosSegment0FirstAlignment();
        readCount++;
    }
    ...
    static void writeRecord(
        ...
        byte recordSegments = record.getNumRecordSegments();
        if(recordSegments > numberTemplateSegments){
            throw new IllegalArgumentException();
        }
        byte alignedSegments = record.getNumAlignedSegments();
        if(dataClass == DATA_CLASS.CLASS_HM && (recordSegments != 2 || alignedSegments != 1)){
            throw new IllegalArgumentException();
        }

        writePairFirstAlignment(
                record,
                dataClass,
                numberTemplateSegments,
                symbolValues,
        ...
        writeRlen(
                ...
                splicedReads,
                dataClass,
                record.getNumRecordSegments(),
                record.getNumAlignedSegments(),
                symbolValues,
                numberSymbols
        );
    ...
    private static void writeQualities(
        ...
        writeQualityAlignedSegment(
                record.getMappingPositions()[0][0],
                record.getOperationLength()[0][0],
                record.getOperationType()[0][0],
                qualityBookIndexes,
                parameterSet,
                record.getQualityValues()[0],
                auStartPosition,
                symbols,
                numSymbols
        );
        if(record.getNumRecordSegments() > 1) {
            if (record.getSplitMate()[0][1] == SplitType.MappedSameRecord) {
                writeQualityAlignedSegment(
                        record.getMappingPositions()[0][1],
                        record.getOperationLength()[0][1],
                        record.getOperationType()[0][1],
                        qualityBookIndexes,
                        parameterSet,
                        record.getQualityValues()[1],
                        auStartPosition,
                        symbols,
                        numSymbols
                );
        ...
        for(int splice_i=0; splice_i < mappingPositions.length; splice_i++){
            ...
            for(int operation_i = 0; operation_i < operations[splice_i].length; operation_i++){
                ...
                for(int operationBase_i = 0;
                    ...
                    short encoded = parameterSet.getQualityBook(codebookId).encode(qualities[0][currentQualityIndex]);
                    addSymbol(encoded,
                              DESCRIPTOR_ID.QV,
                              (byte)(codebookId + 2),
                              symbols,
     ...
     */
    static void writePairFirstAlignment(
            Record record,
        ...
        if(data_class == DATA_CLASS.CLASS_HM){
            if(record.isRead1First()){
                addSymbol(0, DESCRIPTOR_ID.PAIR, (byte)1, symbolValues, numberSymbols);
            } else {
                addSymbol(1, DESCRIPTOR_ID.PAIR, (byte)1, symbolValues, numberSymbols);
            }
        } else {
            for(int i=1; i < templateRecordSegments; i++){
                if(record.getSplitMate()[0][i] == SplitType.MappedSameRecord){
                    addSymbol(0, DESCRIPTOR_ID.PAIR, (byte)0, symbolValues, numberSymbols);
                    if(data_class != DATA_CLASS.CLASS_U){
                        long toWrite = record.getMappingPositions()[0][1][0] - record.getMappingPositions()[0][0][0];
                        toWrite = (toWrite << 1) | (record.isRead1First() ? 1:0);
                        addSymbol(
                                toWrite,
                                DESCRIPTOR_ID.PAIR,
                                (byte)1,
                ...
                } else if (record.getSplitMate()[0][i] == SplitType.MappedDifferentRecordSameSequence){
                    if(!record.isRead1First()){
                        addSymbol(1, DESCRIPTOR_ID.PAIR, (byte)0, symbolValues, numberSymbols);
                        addSymbol(
                                record.getMappingPositions()[0][i][0],
                                DESCRIPTOR_ID.PAIR,
                                (byte)2,
                                symbolValues,
                                numberSymbols
                ...
                } else if(record.getSplitMate()[0][i] == SplitType.UnmappedDifferentRecordSameAU){
                    if(!record.isRead1First()){
                        addSymbol(1, DESCRIPTOR_ID.PAIR, (byte)0, symbolValues, numberSymbols);
                        addSymbol(
                                record.unmappedRecordAUSegment[i],
                                DESCRIPTOR_ID.PAIR,
                                (byte)2,
                                symbolValues,
                                numberSymbols
                ...
                } else if (record.getSplitMate()[0][i] == SplitType.MappedDifferentRecordDifferentSequence){
                    if(!record.isRead1First()){
                        addSymbol(3, DESCRIPTOR_ID.PAIR, (byte)0, symbolValues, numberSymbols);
                        addSymbol(
                                record.getMappingSequence()[0][i].getSequenceIdentifier(),
                                DESCRIPTOR_ID.PAIR,
                                (byte)4,
                                symbolValues,
                                numberSymbols
                ...
                } else if (record.getSplitMate()[0][i] == SplitType.UnmappedDifferentRecordDifferentAU){
                    if(!record.isRead1First()){
                        addSymbol(3, DESCRIPTOR_ID.PAIR, (byte)0, symbolValues, numberSymbols);
                        addSymbol(
                                record.unmappedRecordAUId[i],
                                DESCRIPTOR_ID.PAIR,
                                (byte)4,
                                symbolValues,
                                numberSymbols
                ...
                } else if(record.getSplitMate()[0][i] == SplitType.Unpaired){
                    if(record.isRead1First()){
        ...
        for(int segment_i=0; segment_i < softClips.length; segment_i++) {
            if (softClips[segment_i][0].length != 0) {
                ...
                for (int symbol_i = 0; symbol_i < softClips[segment_i][0].length; symbol_i++) {
                    ...
                    addSymbol(
                            ALPHABET_ID.charToId(alphabet_id, (char) charToRepresent),
                            DESCRIPTOR_ID.CLIPS,
                            (byte) 2,
                            symbolValues,
                            numberSymbols
     ...
     *                     alignment, the third to the splice.
    ...
    static void writeRlen(
            ...
            boolean splicedReads,
            DATA_CLASS data_class,
            int numberRecordSegments,
            int numAlignedRecordSegments,
            long[][][] symbolValues,
            int[][] numberSymbols) {
        ...
        if(readsLengthParameter == 0) {
            for(int segment_i=0; segment_i<numberRecordSegments; segment_i++){

                long totalLengthSegment = 0;
                for(int splice_i=0; splice_i<spliceLength[0][segment_i].length; splice_i++){
                    totalLengthSegment += spliceLength[0][segment_i][splice_i];
                }

                addSymbol(
                        totalLengthSegment - 1,
                        DESCRIPTOR_ID.RLEN,
                        (byte) 0,
                        symbolValues,
                        numberSymbols
        ...
        if(splicedReads && (data_class == DATA_CLASS.CLASS_I || data_class == DATA_CLASS.CLASS_HM)){
            for(int segment_i = 0; segment_i < numAlignedRecordSegments; segment_i++){
                for(int splice_i = 0; splice_i < spliceLength[segment_i][0].length; splice_i++){
                    addSymbol(
                            spliceLength[segment_i][0][splice_i] - 1,
                            DESCRIPTOR_ID.RLEN,
                            (byte) 0,
                            symbolValues,
                            numberSymbols
    ...
    static void writeMMAP(
        ...
        if(dataClass != DATA_CLASS.CLASS_U){
            if(multiple_alignment_flag){
                addSymbol(
                        record.getNumAlignmentsForSegment(0),
                        DESCRIPTOR_ID.MMAP,
                        (byte)0,
                        symbols,
                        numSymbols
                );
            } else if( record.getNumAlignmentsForSegment(0)!=1){
                throw new IllegalArgumentException();
            }
        }

        if(record.isUnpaired() || dataClass == DATA_CLASS.CLASS_HM){
            if(record.getNumAlignmentsForSegment(1) != 0){
                throw new IllegalArgumentException();
            }
            ...
            for(
                    ...
                    alignmentSegment0_i < record.getNumAlignmentsForSegment(0);
                    alignmentSegment0_i++){
                int[] alignmentsSegment1ForAlignmentSegment0 = record.getAlignmentIndexesSegment1(alignmentSegment0_i);
                if(!multiple_alignment_flag) {
                    if(alignmentsSegment1ForAlignmentSegment0.length != 1) {
                        throw new IllegalArgumentException();
                    }
                } else {
                    addSymbol(
                            alignmentsSegment1ForAlignmentSegment0.length,
                            DESCRIPTOR_ID.MMAP,
                            (byte)0,
                            symbols,
                            numSymbols
                ...
                int numCurrentlyWritten = 0;
                for(
                        int alignmentSegment1_i = 0;
                        alignmentSegment1_i < alignmentsSegment1ForAlignmentSegment0.length;
                        alignmentSegment1_i++) {
                    if(alignmentSegment0_i == 0) {
                        if(alignmentsSegment1ForAlignmentSegment0[alignmentSegment1_i] != 0) {
                            throw new IllegalArgumentException();
                        }
                        numCurrentlyWritten = 1;
                    } else {
                        int toWrite = alignmentsSegment1ForAlignmentSegment0[alignmentSegment1_i];
                        if(toWrite > numCurrentlyWritten) {
                            throw new IllegalArgumentException();
                        }
                        int ptr = toWrite - numCurrentlyWritten;
                        if(ptr == 0) {
                            numCurrentlyWritten++;
                        }
                    }
        ...
        if (multiple_alignment_flag && dataClass != DATA_CLASS.CLASS_U){
            addSymbol(
                    record.hasMoreAlignments() ? 1 : 0,
                    DESCRIPTOR_ID.MMAP,
                    (byte)2,
                    symbols,
                    numSymbols
            );
            if(record.hasMoreAlignments()){
                addSymbol(
                        record.getMoreAlignmentsNextSeqId().getSequenceIdentifier(),
                        DESCRIPTOR_ID.MMAP,
                        (byte)3,
                        symbols,
                        numSymbols
                ...
                addSymbol(
                        record.getMoreAlignmentsNextPos(),
                        DESCRIPTOR_ID.MMAP,
                        (byte)3,
                        symbols,
                        numSymbols
    ...
    static void encodeGenomicPosition(
        ...
        addSymbol(positions[0][0][0]-previousPosition, DESCRIPTOR_ID.POS, (byte) 0, symbolValues, numberSymbols);
        for(int alignment_i=1; alignment_i < positions.length; alignment_i++){
            addSymbol(
                    positions[alignment_i][0][0] - positions[alignment_i - 1][0][0],
                    DESCRIPTOR_ID.POS,
                    (byte) 1,
                    symbolValues,
                    numberSymbols
    ...
    static void writePairOtherAlignments(
            DATA_CLASS dataClass,
            Record record,
            long[][][] symbols,
            int[][] numSymbols,
            boolean splicedReads,
            int numberOfTemplateSegments){
        for(
                int alignmentSegment0_i = 1;
                alignmentSegment0_i < record.getNumAlignmentsForSegment(0);
                alignmentSegment0_i++){
            if(record.getSplitMate()[alignmentSegment0_i][0] != SplitType.MappedSameRecord){
                throw new IllegalArgumentException();
            }
            ...
            if(
                    (dataClass == DATA_CLASS.CLASS_P ||
                            ...
                            dataClass == DATA_CLASS.CLASS_I) && !record.isUnpaired()){
                for(int segment_j = 1; segment_j < numberOfTemplateSegments; segment_j++){
                    int currAlignIdx = 0;
                    for(
                            int alignmentSegmentj_i = 1;
                            alignmentSegmentj_i < record.getAlignPtr().length;
                            alignmentSegmentj_i++){
                        int alignIdx = record.getAlignPtr()[alignmentSegmentj_i][segment_j];
                        if(alignIdx > currAlignIdx){
                            currAlignIdx = alignIdx;

                            if(record.getSplitMate()[alignmentSegmentj_i][segment_j] == SplitType.MappedSameRecord){
                                long absoluteValueDelta = Math.abs(
                                        record.getMappingPositions()[alignmentSegment0_i][0][0]
                                        - record.getMappingPositions()[alignmentSegmentj_i][segment_j][0]
                                );
                                if(absoluteValueDelta <= 32767){
                                    addSymbol(
                                            0,
                                            DESCRIPTOR_ID.PAIR,
                                            (byte)0,
                                    ...
                                    long toWrite = absoluteValueDelta << 1;
                                    toWrite |= (record.getMappingPositions()[alignmentSegment0_i][0][0]
                                            < record.getMappingPositions()[alignmentSegmentj_i][segment_j][0]) ? 1 : 0;
                                    addSymbol(
                                            toWrite,
                                            DESCRIPTOR_ID.PAIR,
                                            (byte)1,
                            ...
                            } else if (record.getSplitMate()[alignmentSegmentj_i][segment_j]
                                ...
                                addSymbol(
                                        record.getMappingSequence()[alignmentSegmentj_i][segment_j].getSequenceIdentifier(),
                                        DESCRIPTOR_ID.PAIR,
                                        (byte)5,
                                        symbols,
                                        numSymbols
    ...
    static void writePairSplicingDistances(
            DATA_CLASS dataClass,
            Record record,
            long[][][] symbols,
            int[][] numSymbols){
        int numberOfAlignedRecordSegments = record.getNumAlignedSegments();
        if(dataClass == DATA_CLASS.CLASS_I || dataClass == DATA_CLASS.CLASS_HM){
            for(int segment_i=0; segment_i < numberOfAlignedRecordSegments; segment_i++){
                long[] positions = record.getMappingPositions()[0][segment_i];
                for(int splice_j = 1; splice_j < positions.length; splice_j++){
                    long prevSpliceMappingEnd = positions[splice_j -1] +
                            record.getSpliceLengths()[0][segment_i][splice_j];

                    long absDifference = Math.abs(positions[splice_j] - prevSpliceMappingEnd);
                    if(absDifference <= 32767){
                        long toWrite = absDifference << 1;
                        toWrite |= positions[splice_j] < prevSpliceMappingEnd ? 1 : 0;
        ...
        for(
            ...
            for(
                ...
                for(
                    ...
                    addSymbol(
                            reverseComp[alignedRecordSegment_i][alignment_i][splice_i].id,
                            DESCRIPTOR_ID.RCOMP,
                            (byte)0,
                            symbols,
                            numSymbols
    ...
    private static void writeMMposAndMMtype(
        ...
        for(int alignedSegment_i = 0; alignedSegment_i<record.getNumAlignedSegments(); alignedSegment_i++) {
    ...
    static void writeMMposAndMMtypeSegment(
        ...
        for(int splice_i=0;
                splice_i<record.getOperationType()[alignment_i][segment_i].length;
                splice_i++) {

            byte[] originalOperations = record.getOperationType()[alignment_i][segment_i][splice_i];
            ...
            if(originalOperations[0] == Operation.HardClip) {
                if(originalOperations[1] == Operation.SoftClip) {
                    retainFrom = 2;
                    lengthFirstSoftClip = record.getOperationLength()[alignment_i][segment_i][splice_i][1];
                } else {
                    retainFrom = 1;
                }
            } else {
                if(originalOperations[0] == Operation.SoftClip){
                    retainFrom = 1;
                    lengthFirstSoftClip = record.getOperationLength()[alignment_i][segment_i][splice_i][0];
                }else{
                    retainFrom = 0;
                }
            }
            ...
            try {
                System.arraycopy(
                        record.getOperationLength()[alignment_i][segment_i][splice_i],
                        retainFrom,
                        operationsLength,
                        0,
                        retainTo - retainFrom);
            ...
            long startPos = segment_i == 0 ?
                    record.getMappingPositions()[alignment_i][segment_i][splice_i] :
                    record.getMappingPositions()[alignment_i][segment_i][splice_i];

            long maxLength = record.getSpliceLengths()[alignment_i][segment_i][splice_i];
            ...
            Payload reference = sequencesSource.getSubsequence(
            ...
            offsetDeltaDueToSplice += record.getSpliceLengths()[alignment_i][segment_i][splice_i];
        }
    }
    ...
    static void writeMMposAndMMtypeForOneSegmentOneAlignmentOneSplice(
        ...
        for(int i=firstOperation; i<operations.length; i++) {
            ...
            if(operations[i] == Operation.Substitution || operations[i] == Operation.SubstitutionToN){
                ...
                oldBaseIdentifier = ALPHABET_ID.charToId(
                        alphabet_id,
                        (char)referenceSequence.readByte());

                newBaseIdentifier = ALPHABET_ID.charToId(
                        alphabet_id,
    ...
    private static void writeMscore(
            Record record,
            byte alignedSegments,
            long[][][] symbols,
            int[][] numSymbols
    ) {
        long[][][] mappingScore = record.getMapping_score();
        int as_depth = mappingScore[0][0].length;

        for(int depth_i=0; depth_i < as_depth; depth_i++){
            for(int segment_i=0; segment_i < alignedSegments; segment_i++){
                ...
                for(int alignment_i=0; alignment_i < numAlignmentsForSegment; alignment_i++){
                    if(record.getSplitMate()[alignment_i][segment_i] == SplitType.MappedSameRecord) {
                        addSymbol(
                                mappingScore[alignment_i][segment_i][depth_i],
