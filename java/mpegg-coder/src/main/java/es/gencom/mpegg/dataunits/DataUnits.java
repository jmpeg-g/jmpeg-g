...
public class DataUnits {
    ...
    public void write(final MPEGWriter writer, final MPEGWriter writerRawRef) throws IOException {
        ...
        for(DataUnitAccessUnit dataUnit : dataUnits) {
            System.out.println(dataUnit.getAUType()+"\t"+dataUnit.getId());
    ...
    public DataUnitParameters getParameter(final int id){
        for(DataUnitParameters dataUnitParameters: dataUnitParametersList){
            if(dataUnitParameters.parameter_set_id == id){
    ...
    public String[] getAllReadGroupNames(){
        Set<String> setReadGroupNames = new HashSet<>();
        for(DataUnitParameters dataUnitParameters : dataUnitParametersList){
            String[] readGroupNames = dataUnitParameters.getEncodingParameters().getReadGroupIds();
            Collections.addAll(setReadGroupNames, readGroupNames);
        }
        String[] result = new String[setReadGroupNames.size()];
        result = setReadGroupNames.toArray(result);
        return result;
    }
}
