...
public class HalfMappedAccessUnitEncoder extends AbstractAccessUnitEncoder{
    ...
    protected void writeSpecific(Record record) throws IOException {
        ...
        previousPosition = record.getMappingPositions()[0][0][0];
        readCount++;
    }
    ...
    static void writeRecord(
        ...
        MappedAccessUnitEncoder.writePairFirstAlignment(
                record,
                dataClass,
                numberTemplateSegments,
                symbolValues,
        ...
        MappedAccessUnitEncoder.encodeGenomicPosition(
                record.getMappingPositions(),
                previousPosition,
                symbolValues,
                numberSymbols
        );

        MappedAccessUnitEncoder.writePairOtherAlignments(
                dataClass,
                record,
                symbolValues,
                numberSymbols,
        ...
        MappedAccessUnitEncoder.writePairSplicingDistances(
                dataClass,
                record,
                symbolValues,
                numberSymbols
    ...
    private static void writeUnmappedMate(
        ...
        for(int base_i = 0; base_i < record.getSequenceBytes()[1].length; base_i++){
            byte encodedBase = ALPHABET_ID.charToId(alphabet_id, (char)record.getSequenceBytes()[1][base_i]);
            addSymbol(
                    encodedBase,
    ...
    private static void writeRcompHalfMapped(
        ...
        for(
            ...
            for(
                ...
                addSymbol(
                        reverseComp[0][alignment_i][splice_i].id,
                        DESCRIPTOR_ID.RCOMP,
                        (byte)0,
                        symbols,
                        numSymbols
    ...
    private static void writeQuality(
        ...
        MappedAccessUnitEncoder.writeQualityAlignedSegment(
    ...
    private static void writeMscore(
        ...
        for(int depth_i=0; depth_i < as_depth; depth_i++){
            for(int alignment_i=0; alignment_i < mappingScore.length; alignment_i++){
                if(record.getSplitMate()[alignment_i][0] == SplitType.MappedSameRecord) {
                    addSymbol(
                            mappingScore[0][alignment_i][depth_i],
